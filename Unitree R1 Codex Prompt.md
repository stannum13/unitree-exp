...
Before any campaign resource is created, require a pre-provisioned shared GCP Workflows watchdog and its execution service account. They are operator-owned prerequisites, not campaign-created resources. Verify the exact immutable workflow revision/source hash. The execution service account has no user-managed key and no impersonator other than the Workflows service agent. Its pre-provisioned permissions allow only bounded GET/list/delete for Compute resources satisfying the supplied scope and pre-provisioned disposable Resource Manager tag; it has no VM/disk create, start, stop, suspend, IAM-write, general administration, or `logging.logEntries.create` permission. Its only Storage write permissions are the exact bucket-scoped permissions added in the `ACTIVE` policy below: control-marker creation, temporary-hold clearing, conditional object deletion, and bucket deletion. Audit the identities allowed to invoke the shared workflow and to cancel an execution: only the outside controller principal may start or safely cancel this campaign's exact execution, and the watchdog service account cannot cancel itself. Do not create, update, or delete the shared workflow, its service account, or the pre-provisioned tag during this campaign. If any prerequisite is absent, mutable, over-privileged, unauthorized, or cannot be armed, return `BLOCKED_GCP_WATCHDOG` and create no worker.
...
Use a dedicated worker service account and exact, prefix-conditioned custom roles on this bucket; never grant it bucket configuration/deletion, project Editor, project-wide Storage Admin, or unrelated permissions. The worker uploader may create objects only under `run/<nonce>/artifacts/`, always with `ifGenerationMatch=0`; it may read only what atomic acknowledgement requires and cannot update or delete objects. The watchdog may create objects only under `control/watchdog/<execution-id>/`, also with `ifGenerationMatch=0`; separately it has exact-bucket get/list, object get, temporary-hold update, generation-conditional delete, and bucket delete permissions required for emergency cleanup. The watchdog bucket binding has no expiry before verified bucket `404`; if the platform requires an expiry, it must be strictly later than the artifact emergency deadline plus the predeclared retry allowance. Neither principal can create in the other's namespace. Audit effective permissions and record the two exclusive create scopes in the ownership ledger.

Define exactly two accepted normalized bucket IAM policy states after preserving every pre-existing/inherited policy element byte-for-byte. `ACTIVE` is the frozen baseline plus the exact worker-writer tuple and the exact watchdog control-marker and cleanup tuples described above. `SEALED` is byte-for-byte the same normalized policy except that the one exact worker-writer role/member/condition-title tuple is absent; all watchdog authority remains. Each state's proof records its normalized tuple hash, while a fresh policy-v3 etag is used only as the set-policy concurrency guard. After all worker uploads stop and their acknowledgements are captured, the controller may transition `ACTIVE` to `SEALED` by fresh GET, exact normalized-policy match, and removal of only that worker-writer tuple with the fresh etag. The watchdog accepts exactly `ACTIVE` or `SEALED`, and no third state. Never remove or expire its marker/hold-clear/delete/bucket-delete authority before verified bucket `404`; bucket deletion itself destroys the bucket-scoped bindings.
...
- IAM binding: exact normalized role/member/condition-title tuple against a freshly read policy-v3 etag; remove only that tuple with the etag guard and preserve every other binding byte-for-byte; etag is a concurrency token, not identity;
- bucket: exact globally unique name, owning project number, create receipt, campaign labels, `timeCreated`, location, initially empty all-versions listing, exact `ACTIVE`/`SEALED` normalized-policy hash, and create-ledger identity; current metageneration is only a mutation/delete precondition;
...
10. Create the cost ledger and unique regional campaign-bucket plan, including bucket-empty/name-collision checks, immutable bucket identity versus fresh concurrency tokens, exact `ACTIVE`/`SEALED` policies, disjoint artifact/control-marker create namespaces, the 1,000-generation ceiling, object-generation ownership, temporary holds, Custom-Time lifecycle, quotas, host bind mount, independent uploader, upload/local-recovery acknowledgements, both watchdog deadlines, and resource-specific cleanup ownership.
...
After G0 passes, create only the rendered non-worker prerequisites and record their ownership proofs. Immediately before starting the watchdog, repeat the outside-controller exact-name VM/disk `404` checks. Start the one exact execution with immutable input; install and verify its exact `ACTIVE` bucket policy; require the workflow's independent absence checks and generation/checksum/metadata-verified `ARMED_PRECREATE` control marker; repeat exact-name `404` checks; and only then issue the sole requestId-bound VM insert. After the sole worker becomes billable, keep bootstrap paused until the verified `ARMED_COMPUTE` marker; then:
