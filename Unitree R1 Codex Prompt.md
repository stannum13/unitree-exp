...
```json
{
  ...
  "metadata": {"items": [
    ...
    {"key": "startup-script", "value": "<exact-startup-script-content>"}
  ]},
  "disks": [{
    "boot": true,
    "autoDelete": true,
...
Standard on-demand is mandatory. Permit at most a primary capacity attempt plus one alternate-zone attempt. The alternate is allowed only after the first ends with an enumerated terminal non-billable capacity error, complete proof that no VM, disk, or other billable state ever existed, and verified cleanup/cancellation of every first-attempt bucket, execution, network resource, service account, and object generation. It uses a fresh independent 128-bit nonce, requestId, bucket, execution, deadlines, rendered body, and full preflight. Any ambiguity, survivor, or billable state permanently forbids the alternate. Across both attempts, allow at most one live attempt, one worker ever billable, one successful worker, 1,000 object generations, 20,000 workflow internal steps, and the existing USD 0.50 watchdog/API sub-bound. All first-attempt cost enters cumulative realized cost. Do not create a replacement after bootstrap failure, user-code failure, deletion, or any billable start.
...
For each attempt calculate exactly:
...
authorized_compute_hours = min(
    23.75,
    floor_to_0.1h(
        max(0, (remaining_envelope - remaining_nonruntime_reserve)
               / conservative_all_in_hourly_rate - 0.25)
    )
)
```

`conservative_all_in_hourly_rate` is the greater of the live applicable rate and the design-time on-demand rate. Every prior capacity-attempt, API, workflow, storage, network, and other campaign cost enters the realized totals. The `0.25h` deletion allowance is subtracted exactly once here and nowhere else. With zero realized cost and the design-time rate, the expression authorizes `23.7` hours after the 0.1-hour floor, within the `23h45m` absolute deadline; the 24-hour rate bound remains a conservative envelope. Reduce runtime when the bound rises. The `3.899852512 USD` contingency is never runtime, and no incidental discount expands runtime.
...
- `SCREEN_FUTILE_NO_CONFIRM`: real SCREEN execution completed, every challenger's posterior-predictive probability that a fresh confirmation panel would satisfy the posterior component of `PILOT_SUCCESS`—`P(Delta > 0) >= 0.95` and `P(Delta >= MES) >= 0.50`—was below 0.10, no CONFIRM data was opened, and the incumbent was retained. This is an operational SCREEN disposition, not an efficacy or equivalence claim; record `implementation_status=PASS`, `scientific_status=SCREEN_FUTILE_NO_CONFIRM`, and return the saved confirmation budget to adaptive selection.
- `SCREEN_DIAGNOSTIC_REJECT_NO_CONFIRM`: real SCREEN execution completed; at least one challenger had posterior-predictive probability at or above 0.10 that a fresh confirmation panel would satisfy the posterior component of `PILOT_SUCCESS`—`P(Delta > 0) >= 0.95` and `P(Delta >= MES) >= 0.50`—but every predictive survivor failed at least one predeclared non-safety mechanism or Pareto diagnostic; no CONFIRM data was opened; and the incumbent was retained. Record `implementation_status=PASS` and `scientific_status=SCREEN_DIAGNOSTIC_REJECT_NO_CONFIRM`. Safety and mechanism gates remain separate from the posterior component. This is an operational/diagnostic disposition only and supports no efficacy, inferiority, equivalence, or CONFIRM-level `FUTILE` or `INCONCLUSIVE` claim.
...
After G0 passes, create only the rendered per-attempt non-worker resources and record their ownership proofs. Create the new bucket in exact `BASE`; repeat the outside-controller exact-name VM/disk `404` checks; start the attempt's exact execution with concurrency overflow/backlogging disabled and `LOG_NONE`; capture and poll its exact resource name and immutable revision to `ACTIVE`, rejecting `QUEUED`; use that execution ID to install and verify the exact `ACTIVE` direct-policy hash and effective-access audit; require the already-active workflow's independent absence checks and generation/checksum/metadata-verified `ARMED_PRECREATE` marker within its bounded arming interval; repeat exact-name `404` checks; and only then issue that attempt's requestId-bound Compute v1 REST insert. After the sole worker becomes billable, keep bootstrap paused until the verified `ARMED_COMPUTE` marker; then:
