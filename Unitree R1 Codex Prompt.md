...
Before any campaign resource is created, require a pre-provisioned shared GCP Workflows watchdog and its execution service account. They are operator-owned prerequisites, not campaign-created resources. Verify the exact workflow revision/source hash. The execution service account has no user-managed key and no impersonator other than the Workflows service agent. Its bounded permissions allow only GET/list/delete for the supplied compute, disk, and storage scope plus object-metadata update solely to clear a temporary hold; it has no create, VM start/stop, IAM-write, or general administration permission. Do not create, update, or delete the shared workflow or its service account during this campaign. If either prerequisite is absent, mutable, over-privileged, unauthorized, or cannot be armed, return `BLOCKED_GCP_WATCHDOG` and create no worker.

Compute and record two immutable UTC RFC3339 deadlines before launch. The hard compute deadline is no later than the VM-create request plus `23h45m`, or earlier when the authorized runtime calculation requires it. The artifact emergency deadline is no later than seven days after that create request, fitting the declared storage/operations reserve. Start exactly one watchdog execution before sending the VM-create request, passing the campaign nonce; exact project, zone, intended VM and boot-disk names; exact bucket containment proof; all other intended campaign resource identities; both deadlines; and the workflow revision. Require a durable structured `ARMED_PRECREATE` workflow log containing the exact execution name, revision, nonce, scope, and deadlines before any paid VM create, and copy that log identity into the outside ledger. The execution must poll for the VM and emit a second durable `ARMED_COMPUTE` log containing the server-assigned VM numeric ID, creation timestamp, insert-operation name/ID, operation `targetId`/`targetLink`, and the boot disk's numeric ID, boot status, and auto-delete setting. Apart from the minimum interval needed to create and identify the worker, no bootstrap or experimental paid work may proceed until that second acknowledgement is verified and copied into the ledger. Failure to receive either acknowledgement triggers immediate ownership-checked controller deletion and `BLOCKED_GCP_WATCHDOG`.

The rendered VM create command must use `--termination-time=<hard-compute-deadline>`, `--instance-termination-action=DELETE`, and `--no-restart-on-failure`; it must not configure a restart-relative duration fuse. After creation, mechanically require `automaticRestart=false`, require the server-reported `terminationTimestamp` to represent the same normalized UTC instant, and verify boot-disk auto-delete. This is only a secondary `RUNNING`-state fuse: it does not cover `STOPPED` or `SUSPENDED`, where Compute Engine may remove the termination timestamp.
...
At the hard compute deadline, the watchdog wakes and GET-validates the exact project/zone/name, numeric ID, creation timestamp, and insert-operation target identity against its immutable execution scope. A matching `404` after a previously captured identity is an idempotent already-deleted result; an extant same-name resource with a different identity is an ownership mismatch. Otherwise it deletes the VM even when it is `STOPPED`, using a deterministic idempotent request ID, and waits for the Compute Engine long-running operation to finish. It then GET-validates the captured boot-disk identity, requires that the disk has no unexpected users, deletes any survivor with a separate deterministic request ID, and waits for that operation. It fails closed on any identity mismatch, records the mismatch, and never widens or guesses a target. The outside controller remains responsible for earlier orderly cleanup and reconciliation; the watchdog is the controller-independent hard stop.
...
Before every paid launch, append a pre-launch ledger record containing every intended resource identity and its resource-specific ownership proof, rate source and retrieval time, conservative hourly rate, fixed cost, authorized runtime interval, hard compute and artifact emergency deadlines, watchdog execution identity/revision/acknowledgements, worst-case incremental cost, cumulative worst-case cost, remaining authorized cost, and deletion allowance. Billing alerts are supplemental and are not spend caps.
...
At the artifact emergency deadline, the watchdog must independently validate the exact bucket containment proof and list every version. A matching `404` after the captured create receipt is an already-deleted result; an extant same-name bucket with a different metageneration/creation identity or writer policy is an ownership mismatch. Refuse to widen cleanup if the listing exceeds the enforced 1,000-generation bound. For each listed post-start generation, capture exact bucket/name/generation/metageneration, CRC32C, and custom run metadata in the execution ledger. The containment proof authorizes emergency handling of a post-start generation even when an interrupted uploader omitted it from its outside ledger. Release any temporary hold with generation and metageneration preconditions, then delete with a generation-match precondition. Verify an empty all-versions listing before deleting the exact bucket with its metageneration precondition. At this deadline the USD 30 ceiling and bounded-resource rule outrank artifact retention: if local recovery is incomplete, record evidence loss when possible and conditionally delete rather than retain held objects or a bucket indefinitely. The workflow is capped at 20,000 internal steps and the bucket at 1,000 generations; any live worst-case bound that violates either cap blocks launch. The single watchdog execution then completes naturally. Execution-history expiry is provider-owned; it does not authorize a campaign Scheduler, Cloud Task, workflow definition, workflow service account, or other persistent watchdog resource.
...
- VM and disk: exact project, zone, name, server-assigned numeric ID, and create-operation/ledger identity;
...
- IAM binding: exact role/member/condition-title tuple against a freshly read policy-v3 etag; remove only that tuple with the etag guard and preserve every other binding byte-for-byte;
- bucket: exact globally unique name, create receipt, campaign label, metageneration, creation time, initially empty all-versions listing, exclusive-writer policy, and create-ledger identity;
...
- watchdog execution: exact execution name, workflow revision, nonce, scope, and both immutable deadlines.

Before each mutation, GET and match every field in that resource-specific proof. A mismatch fails closed, records the precise blocker, and never authorizes a broader search or target; the controller and watchdog can still clean independently owned resources whose proofs match. Delete and verify `DELETED` in dependency order: optional streaming firewall rules; VM and auto-delete boot disk; remaining campaign firewall rules; subnet; VPC; campaign-created IAM bindings; service account; generation-recorded bucket objects; and, only after an empty all-versions listing, the campaign bucket. The normal controller performs this full sequence as early as possible. The armed watchdog independently guarantees VM/disk deletion at the compute deadline and object/bucket deletion at the artifact emergency deadline; a resumed controller reconciles any non-billable survivors using the same proofs. Remove any other campaign-created disposable resource at its dependency-correct point and record create/delete timestamps and API outcomes. Docker/container state is deleted with the boot disk. Never report completion while a disposable resource outcome is merely stopped, suspended, absent from one listing, pending, or an undefined "final state."
...
10. Create the cost ledger and unique regional campaign-bucket plan, including bucket-empty/name-collision checks, the 1,000-generation ceiling, object-generation ownership, temporary holds, Custom-Time lifecycle, quotas, host bind mount, independent uploader, upload/local-recovery acknowledgements, both watchdog deadlines, and resource-specific cleanup ownership.
...
After G0 passes, create only the rendered non-worker prerequisites and record their ownership proofs, then start and verify the watchdog execution exactly as specified before VM creation. After the sole worker becomes billable, wait for `ARMED_COMPUTE`; then:
...
- cloud create-spec invariants including hard compute and artifact emergency deadlines, `automaticRestart=false`, watchdog pre-create/armed acknowledgement, identity-sensitive `404`, stopped-VM deletion, idempotent request IDs, LRO waits, disk-user check, 1,000-generation/20,000-step/USD-0.50 bounds, cost arithmetic, single-worker fuse, allowed firewall sources/ports, unique-bucket containment and exclusive-writer preconditions, resource-specific ownership proofs, fail-closed precondition mismatches, and dependency-ordered cleanup targets without paid mutation.
